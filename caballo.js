...
pasito.onclick = function(){
    ...
    if(step <= n*n){
        ...
        {
            var celdita = document.getElementById(step-1);
            
            celdita.className=celdita.className.replace( /(?:^|\s)caballo(?!\S)/g ,'' )
            celdita.innerHTML=(step-1);
            
        }
       
       

       var celdita = document.getElementById(step);
       celdita.innerHTML=(step);
       
       celdita.className +=  ' caballo';
...
solito.onclick = function(){
  ...
  step=0;
  timer = setInterval(pasito.onclick,100);
}
